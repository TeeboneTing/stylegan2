...
def G_mapping(
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    # Mapping layers.
    # While transfer learning plan E, no fix on mapping layer
    for layer_idx in range(mapping_layers):
        with tf.variable_scope('Dense%d' % layer_idx):
            fmaps = dlatent_size if layer_idx == mapping_layers - 1 else mapping_fmaps
            x = apply_bias_act(dense_layer(x, fmaps=fmaps, lrmul=mapping_lrmul,trainable=True), act=act, lrmul=mapping_lrmul, trainable=True)
...
def G_synthesis_stylegan2(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.variable_scope('4x4'):
        with tf.variable_scope('Const'):
            x = tf.get_variable('const', shape=[1, nf(1), 4, 4], initializer=tf.initializers.random_normal(), trainable=True)
            x = tf.tile(tf.cast(x, dtype), [tf.shape(dlatents_in)[0], 1, 1, 1])
        with tf.variable_scope('Conv'):
            x = layer(x, layer_idx=0, fmaps=nf(1), kernel=3, trainable=True)
        if architecture == 'skip':
            y = torgb(x, y, 2, trainable=True)
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res < resolution_log2:
                ...
                x = block(x, res, trainable=True) # for transfer learning plan E, no fix weight on synthesis layers
            if architecture == 'skip':
                y = upsample(y)
            if architecture == 'skip' or res == resolution_log2:
                if res < resolution_log2:
                    y = torgb(x, y, res, trainable=True)
                else:
                    y = torgb(x, y, res, trainable=True) # for transfer learning plan E, no fix weight on synthesis layers
    images_out = y
...
def D_stylegan2(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    x = None
    y = images_in
    for idx,res in enumerate(range(resolution_log2, 2, -1)):
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if architecture == 'skip' or res == resolution_log2:
                x = fromrgb(x, y, res, trainable=trainable_flag)
            if idx <= 3: # 0 ~ 3 layers fix weight
                x = block(x, res, trainable=trainable_flag)
            else:
                x = block(x, res, trainable=True) # for transfer learning plan E, freeze first 4 layers in D
            if architecture == 'skip':
                y = downsample(y)

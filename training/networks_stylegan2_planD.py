    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    if label_size:
        with tf.variable_scope('LabelConcat'):
            w = tf.get_variable('weight', shape=[label_size, latent_size], initializer=tf.initializers.random_normal(),trainable=True)
    ...
    # Mapping layers.
    # While transfer learning plan D, train early layers
    for layer_idx in range(mapping_layers):
        with tf.variable_scope('Dense%d' % layer_idx):
            ...
            if layer_idx <= 3:
                ...
                x = apply_bias_act(dense_layer(x, fmaps=fmaps, lrmul=mapping_lrmul,trainable=False), act=act, lrmul=mapping_lrmul, trainable=False)
    # Broadcast.
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res < resolution_log2:
                ...
                x = block(x, res, trainable=False) # for transfer learning plan D, fix last block
            if architecture == 'skip':
                y = upsample(y)
            if architecture == 'skip' or res == resolution_log2:
                if res < resolution_log2:
                    ...
                    y = torgb(x, y, res, trainable=False) # for transfer learning plan D, fix last block
    images_out = y
